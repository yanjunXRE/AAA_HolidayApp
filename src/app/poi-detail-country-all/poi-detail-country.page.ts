...
private markers = [];
  constructor(private cdr: ChangeDetectorRef,private activatedRoute: ActivatedRoute, private poi:PoiService, private router: Router ,private geolocation: Geolocation,
  ...
  ngOnInit() {
     ...
     this.poi.loadAll().subscribe(selected=> {
      ...
      if(this.select.images[0].source_url==undefined){
      this.img=this.select.images[0].source_url
      }
    }catch(error){
      this.img=null
    }
